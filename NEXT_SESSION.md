...
- **`_validate_path` fix** (`document_server.py`) — added `/host/home` and `/host/volumes` as allowed Docker mount roots; was blocking all user files → 0 chunks
- **PermissionError fix** (`_index_folder_async` in `app.py`) — file scanner now skips macOS `._*` resource forks with try/except
- **`client/requirements.txt`** — added `pymupdf`, `pymupdf4llm`, `pypdf`, `openpyxl` (missing packages that caused Batch Processing "No module" errors)
...
- Batch Processing calls `_extract_file()` directly in Flask using `pymupdf4llm`.
- KB is for RAG chat. Batch is for offline document extraction (e.g., preprocessing before loading into another system).
